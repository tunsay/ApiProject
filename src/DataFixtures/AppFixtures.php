...
class AppFixtures extends Fixture
{
    ...
    public function loadAdherent(){
        ...
        $adherentAdmin->setNom('Saygili')
                    ->setPrenom('Tuna')
                    ->setMail('admin@gmail.com')
                    ->setPassword($this->passwordEncoder->encodePassword($adherentAdmin,'tuna'))
                    ->setRoles($rolesAdmin);
        $this->manager->persist($adherentAdmin);

        $adherentManager= new Adherent();
        $rolesManager[]=ADHERENT::ROLE_MANAGER;
        $adherentManager->setNom('Vasseur')
                    ->setPrenom('VictorBBGG')
                    ->setMail('manager@gmail.com')
                    ->setPassword($this->passwordEncoder->encodePassword($adherentManager,'Vasseur'))
